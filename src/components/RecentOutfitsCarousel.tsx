...
const RecentOutfitsCarousel = ({
  outfits
}: RecentOutfitsCarouselProps) => {
  ...
  return <div className="overflow-x-auto scrollbar-hide">
      ...
      width: `${outfits.length * 220 + 24}px`
        ...
        {outfits.map((outfit, index) => <div key={outfit.id || index} className="flex-shrink-0 w-52" onClick={() => navigate('/outfits')}>
            <Card className="w-full bg-white border border-gray-100 cursor-pointer hover:shadow-lg transition-all duration-300 hover:scale-105 rounded-2xl overflow-hidden group">
              <CardContent className="p-0">
                <div className="aspect-[4/5] bg-gray-50 overflow-hidden relative">
                  ...
                  {outfit.image_url ? <img src={outfit.image_url} alt={outfit.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" onError={e => {
              ...
              }} /> : outfit.generated_image ? <img src={outfit.generated_image} alt={outfit.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" onError={e => {
              ...
              }} /> : outfit.reference_images && outfit.reference_images.length > 0 ? <img src={outfit.reference_images[0]} alt={outfit.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" onError={e => {
