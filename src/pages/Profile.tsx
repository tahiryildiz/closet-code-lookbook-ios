...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      const {
        count: outfitsCount
      } = await supabase.from('outfits').select('*', {
        count: 'exact',
        head: true
      }).eq('user_id', user.id);
      ...
      const {
        data: favoriteData
      } = await supabase.from('clothing_items').select('*').eq('user_id', user.id).eq('is_favorite', true).limit(5);
  ...
  return <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 pb-20">
      {/* Header */}
        ...
        <div className="px-6 pt-12 pb-8">
          <div className="text-left">
            <div className="w-20 h-20 bg-white/20 rounded-full mb-4 flex items-center justify-center backdrop-blur-sm relative">
              ...
              {limits.isPremium && <div className="absolute -top-1 -right-1 bg-yellow-400 rounded-full p-1">
                  <Crown className="h-4 w-4 text-yellow-800" />
                </div>}
            </div>
            <h1 className="text-2xl font-semibold">
              Profil
              {limits.isPremium && <span className="text-yellow-300 text-sm ml-2">Premium</span>}
            </h1>
            <p className="text-blue-100 text-base mt-1">{user?.email}</p>
            {userProfile?.gender && <p className="text-blue-200 text-sm mt-1">{getGenderDisplay(userProfile.gender)}</p>}
          </div>
        </div>
      </div>

      {/* Profile Content */}
      <div className="py-6 space-y-6 px-[2px]">
        {/* Stats Cards */}
        ...
        {!limits.isPremium && <Card className="bg-gradient-to-br from-purple-500 to-blue-600 text-white border-0 shadow-lg rounded-2xl hover:shadow-xl transition-all duration-300">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <Button onClick={() => setShowPaywall(true)} className="bg-white text-purple-600 hover:bg-gray-100 rounded-xl font-semibold">
        ...
        <div className="space-y-3">
          ...
          <Card className="bg-white/90 backdrop-blur-sm border-0 shadow-lg rounded-2xl hover:shadow-xl transition-all duration-300">
            <CardContent className="p-0">
              <button className="w-full p-4 flex items-center justify-between hover:bg-red-50/50 rounded-2xl transition-colors" onClick={handleFavoritesClick}>
                <div className="flex items-center space-x-3">
        ...
        <Card className="bg-white/90 backdrop-blur-sm border-0 shadow-lg rounded-2xl hover:shadow-xl transition-all duration-300">
          <CardContent className="p-4">
            <Button onClick={handleLogout} disabled={loading} variant="outline" className="w-full border-red-200 text-red-600 hover:bg-red-50 hover:text-red-700 rounded-xl py-3 font-semibold transition-all duration-300">
      ...
      {/* Paywall Modal */}
      <PaywallModal isOpen={showPaywall} onClose={() => setShowPaywall(false)} reason="upgrade" />
    </div>;
};
