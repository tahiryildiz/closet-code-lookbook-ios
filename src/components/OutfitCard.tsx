...
const OutfitCard = ({
    ...
    const fetchItemBrands = async () => {
      if (outfit.item_ids && outfit.item_ids.length > 0) {
        try {
          const {
            data: items
          } = await supabase.from('clothing_items').select('name, brand').in('id', outfit.item_ids);
          if (items) {
  ...
  const hasVerticalFlatlay = outfit.generated_image && (outfit.composition_type === 'professional_flatlay_vertical' || outfit.composition_type === 'professional_flatlay');
  const hasReferenceImages = outfit.reference_images && outfit.reference_images.length > 0;

  // Display logic based on available images
  const primaryImage = hasVerticalFlatlay ? outfit.generated_image : hasReferenceImages ? outfit.reference_images![0] : 'https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=400&h=500&fit=crop';
  ...
  return <Card className="bg-white border-0 shadow-sm hover:shadow-lg transition-all duration-300 rounded-2xl overflow-hidden">
      <CardContent className="p-0">
        {/* Outfit Preview */}
        <div className={`relative overflow-hidden ${hasVerticalFlatlay && (outfit.aspect_ratio === '1024x1536' || outfit.aspect_ratio === '1024x1792') ? 'aspect-[2/3]' // Vertical aspect ratio for supported dimensions
      : 'aspect-[4/5]' // Default aspect ratio
        ...
        <div className="w-full h-full grid grid-cols-2 gap-1 p-2 bg-gray-50">
              {outfit.reference_images!.slice(0, 4).map((imageUrl, index) => <div key={index} className={`relative overflow-hidden rounded-lg bg-white shadow-sm ${outfit.reference_images!.length === 3 && index === 0 ? 'col-span-2' : ''}`}>
                  <img src={imageUrl} alt={`${itemsWithBrands[index] || 'Outfit item'}`} className="w-full h-full object-cover" onError={e => {
        ...
        <div className="w-full h-full">
              <img src={primaryImage} alt={outfit.name} className="w-full h-full object-cover" onError={e => {
          ...
          <div className="absolute top-4 right-4 flex space-x-2">
            <Button size="sm" variant="ghost" className="h-10 w-10 p-0 bg-white/90 hover:bg-white rounded-full shadow-sm" onClick={handleSave} disabled={isSaved}>
              <Heart className={`h-5 w-5 ${isSaved ? 'text-red-500 fill-red-500' : 'text-gray-600 hover:text-red-400'}`} />
            </Button>
            <Button size="sm" variant="ghost" className="h-10 w-10 p-0 bg-white/90 hover:bg-white rounded-full shadow-sm" onClick={handleShare}>
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1">
              {itemsWithBrands.map((item, index) => <div key={index} className="text-sm text-gray-600 flex items-center">
          ...
          {additionalSuggestions.length > 0 && <div className="space-y-2">
              <h4 className="text-base font-semibold text-gray-900">Bu kombine eklenebilecekler:</h4>
              <div className="space-y-1">
                {additionalSuggestions.map((suggestion, index) => <div key={index} className="text-sm text-gray-500 flex items-center">
          ...
          {outfit.styling_tips && !outfit.styling_tips.toLowerCase().includes('this outfit') && <div className="bg-blue-50 rounded-xl p-4">
              <div className="flex items-start space-x-3">
                <div className="bg-blue-100 rounded-full p-2">
                  <Heart className="h-4 w-4 text-blue-600" />
                </div>
                <div className="flex-1">
                  <p className="text-sm font-semibold text-blue-900 mb-1 text-left">Stil İpucu</p>
                  <p className="text-sm text-blue-800 leading-relaxed text-left">{outfit.styling_tips}</p>
                </div>
              </div>
            </div>}
        </div>
