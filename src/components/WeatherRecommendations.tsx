...
const WeatherRecommendations = () => {
  ...
  useEffect(() => {
    const fetchWeatherAppropriateItems = async () => {
      ...
      try {
        ...
        const {
          data: items
        } = await supabase.from('clothing_items').select('id, name, category, primary_color, brand, image_url, material').eq('user_id', user.id);
  ...
  const filterItemsByWeather = (items: WardrobeItem[], weather: WeatherData): WardrobeItem[] => {
    ...
    return items.filter(item => {
      ...
      if (temp >= 25) {
        ...
        const mildWeatherCategories = ['tshirt', 'shirt', 'pants', 'jeans', 'dress', 'skirt', 'light_jacket', 'cardigan', 'sneakers', 'flats'];
        if (mildWeatherCategories.includes(item.category)) return true;
      } else if (temp < 15) {
        // Cold weather - prioritize warm clothing
        const coldWeatherCategories = ['sweater', 'hoodie', 'jacket', 'coat', 'pants', 'jeans', 'boots', 'long_sleeve', 'cardigan'];
    ...
    return <div>
        ...
        <div className="text-center py-12 bg-white rounded-2xl border border-gray-100">
          ...
          <p className="text-gray-500 text-sm">
            {currentWeather ? `${currentWeather.temp}°C ${getWeatherDescription()}${getConditionAdvice()} hava için uygun kıyafet bulunamadı` : 'Hava durumu önerileri için gardırobunuza kıyafet ekleyin'}
          </p>
        </div>
      </div>;
  }
  return <div className="bg-blue-50">
      ...
      <div className="overflow-x-auto scrollbar-hide">
        ...
        width: `${recommendedItems.length * 220 + 24}px`
      }}>
          {recommendedItems.map(item => <div key={item.id} className="flex-shrink-0 w-52" onClick={() => navigate('/wardrobe')}>
